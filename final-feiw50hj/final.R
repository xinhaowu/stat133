...
name <- "Xinhao Wu"
github.name <- "xinhaowu"
email.address <- "xinhao.wu@berkeley.edu"
...
y <- list()
for (i in 1:10) y[[i]] <- seq(from = i, length.out = floor(1000/i), by = i)
...
set.seed(42)
#z <- <your code here>
z = sample(x = c(TRUE,FALSE),prob = c(0.3, 0.7), size = 5000, replace = TRUE)  
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
#v <- <your code here>
v <- sample(6,size = 100, replace = TRUE)
...
#cw4 <- <your code here>
cw4 <- sample(ChickWeight$weight, 200)  
...
# 
# t1 <- <your code here>
t1 <- with(infants, table(smoke,number))
...
plot(mtcars$disp~mtcars$wt, col = mtcars$am+2, xlab = "weight of the car in lb/1000", ylab = "engine displacement in cu. in.")
...
plot(life.expectancy~fertility.rate, data = WorldBank, pch = ".", col = region)
abline(v = 2.1)
...
par(mfrow = c(1,2))
plot(life.expectancy~fertility.rate, data = WorldBank[WorldBank$year == 1960,], pch = ".", col = region)
abline(v = 2.1)
plot(life.expectancy~fertility.rate, data = WorldBank[WorldBank$year == 2013,], pch = ".", col = region)
abline(v = 2.1)
# [4 pts]
...
WBgdp = WorldBank$GDP[WorldBank$lending == "IDA"]
hist(WBgdp,breaks = (max(WBgdp,na.rm = TRUE) - min(WBgdp,na.rm = TRUE))/250)
...
absdiff <- function (x) max(abs(x-mean(x)))
max.diff.rain <- sapply(rain, absdiff)
...
raindays <- function(x) length(x[x>0])
prop.rain <- sapply(rain,raindays) 
...
GenNorm <- function(mean = 0,sd = 1, n = 1000, plot.hist = TRUE){
  ...
  result <- rnorm(n = n, mean = mean, sd = sd)

  if (plot.hist) hist(result)
...
standardize <- function(x) (x-mean(x))/sd(x)
 ...
 standardizeVar <- function(m, cols=TRUE){
   ...
   if (cols)  for (i in 1: dim(m)[2]) m[,i] <- standardize(m[,i])
   else       for (i in 1: dim(m)[1]) m[i,] <- standardize(m[i,])
...
diff.func <- function(cases, controls) {
  
  j <- length(cases)
  m <- length(controls)
  newsample <- sample(c(cases,controls))
  cases <- newsample[1:j]
  controls <- newsample[(j+1):(j+m)]
  return(mean(cases)-mean(controls))
}

PermDiff <- function(cases, controls, k=5000){

return(replicate(k,diff.func(cases = cases,controls = controls)))  
...
OneJackpot <- function(k){
  ...
  for (i in 1:k){
  ticket <- sample(19,3)
  if (sum(match(ticket, win.idx), na.rm = TRUE) == 1) winner.count <- winner.count +1
  }
...
NumJackpot <- function(k,B){
  ...
  return(replicate(B,OneJackpot(k=k)))
}
...
text5 <- grep("[[:punct:]]", phrases, ignore.case = TRUE, value = TRUE)


# [2 pts]
# Create a vector [phrases2] where you have replaced the first instance of the letter "a" in each word with "@"
tmphr <- strsplit(phrases, split = "")
phr.idx <- sapply(gregexpr("a", phrases),"[[",1)
phr.idx[phr.idx == -1] <- 0

for (i in 1:length(phrases)) tmphr[[i]][phr.idx[i]] <- "@"

phrases2 <- sapply(tmphr,paste,collapse = "")
...
# dna2 <- <your code here>
dna2 <- dna[sapply(strsplit(dna,split = ""),length) %%3 == 0]
#################################################################
