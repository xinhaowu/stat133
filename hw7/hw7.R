...
speechYr <- as.numeric(sapply(strsplit(tempDates, split = " "),"[",3))
speechMo <- sapply(strsplit(tempDates, split = " "),"[",1)
...
breaksx <- c(breaks, length(speeches)-1)
speechesL <- list()
for(i in 1:n.speeches){
  
        bodyofspeech <- speeches[c(breaksx[i]+5):c(breaksx[i+1]-1)]
        bodyofspeech <- paste(bodyofspeech, collapse = " ")
        speechesL[i] <- strsplit(bodyofspeech, split = "[.?!]")
...
wordVecs <- lapply(speechesL, speechToWords)
WordCounts <- lapply(wordVecs,table)
Expendtable <- function(table) {
        ...
        for (i in 1:length(table)){
                ...
                emptyVec[names(table)[i]] = table[names(table[i])]
 
        }
        return(emptyVec)
}
WordCounts <- lapply(WordCounts,Expendtable)

# Create a matrix out of wordVecs:
# wordMat <- <your code here>
wordMat <- matrix(WordCounts[[1]])

for(i in 2:n.speeches) wordMat <- cbind(WordCounts[[i]], wordMat)
...
chars <- sapply(speechWords, function(x) length(unlist(strsplit(x, split=""))))
...
trans.wordMat <- t(wordMat)
rownames(trans.wordMat) <- presidents
presidentWordMat <- t(aggregate(trans.wordMat, list(President = presidents), sum))
presidentWordMat <- apply(presidentWordMat[-1,], c(1,2), as.numeric)
colnames(presidentWordMat) <- sort(unique(presidents))
...
president.freq <- presidentWordMat
president.freq[president.freq != 0] <- 1
docFreq <- apply(president.freq, 1, sum)
...
rownames(presDist) <- sort(unique(presidents))
...
presparty0 <- speechesDF[,c(1,3)]
presparty01 <- unique(presparty0)$Pres
names(presparty01) <-unique(presparty0)$party
presparty02 <- sort(presparty01)
presParty <- names(presparty02)
names(presParty) <- presparty02

# use rainbow() to pick one unique color for each party (there are 6 parties)

cols <- rainbow(6)
names(cols) <- unique(speechesDF$party)
...
text(mds,sort(unique(presidents)),srt = 90, cex = 0.75, 
     col = cols[presParty[rownames(presDist)]])
